...
main
 ...
 rem reset if bug 
 rem reset player position if edges to prevent cheating
 rem **********************************************
 ...
 rem * Reset if bug 
 if joy0fire then c=(rand/2)+1 : d=(rand/2)+1 : e=(rand/4)+1
 rem * Reset player position to prevent cheating 
 if player0y<8 then b=(rand/4)+1
 if player0y>84 then b=(rand/4)+1 
 if player0x<8 then a=(rand/4)+1
 if player0x>150 then a=(rand/4)+1

 rem **********************************************
 rem controls for asteroid 
 rem correct stucking
 rem fixing position
 rem **********************************************
 ...
 rem * Corrects the asteroid if stuck 
 if player1x=player1y || e<32 then c=c-1 : d=d+1 : sounda=32: soundb=55
 if player1x=player1y || e>32 then c=c+1 : d=d-1 : sounda=55: soundb=32
 rem * Fix asteroid position 
 ...
 rem * If player touch the asteroid loses 
 if collision(player0,player1) then c=(rand/2)+1 : d=(rand/2)+1 : a=80 : b=50 : goto lose
